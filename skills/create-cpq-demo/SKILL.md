...
6. Build `src/components/ConfiguratorDemo.jsx` to mirror that structure as closely as reasonably possible: tabs for top-level groups, sections within tabs, fields laid out in the same columns/order, and a sensible React control per widget type — all wired to the existing `fields`, `update`, and `updatePickerSelect` from `useConfigurator()`. Keep the BOM sidebar and Submit Quote button from the base template; they don't come from the layout. `FieldControl.jsx` (bundled in this template) is a generic per-field renderer driven by `dataType`/`optionSet` — reuse it rather than writing bespoke controls per field, and pass it a per-field config object (name, label, and any widget override) rather than hand-writing JSX per field. **`MultiSelectProductPickerGrid` is not the same thing as `MultiSelectPicklist`**, even though both show up as `dataType: "array"` on the wire — a `MultiSelectProductPickerGrid` field has a `field.rows.content` structure and each row must be toggled via `updatePickerSelect('<name>.select', selected, name, row.index)`, not via a plain `update(name, newArray)` call (that gets silently ignored, no error). Render `MultiSelectProductPickerGrid` fields yourself as a row list outside of `FieldControl` — don't route them through its generic array/checkbox branch, which is only correct for genuine `MultiSelectPicklist` fields.
9. **Actually wire up `handleSubmit`** — don't leave the template's `items: []` placeholder in place. Map `products` (the BOM) into the full-tree shape the bundled ServiceNow route expects: `{ id, name, parentProduct, isRoot, price, quantity, uniqueIdentifier }` for every product, not a pre-filtered list (see the comment in the template's `handleSubmit` for the exact mapping). The route itself is what excludes container-node pricing and links the hierarchy via `parent_line_item`/`top_line_item` — sending it a flattened or filtered list breaks that.
...
8. **Render rule messages under the specific field they're about, not in one generic list at the top of the page.** Each entry in the session's `messages` array carries a `field` (which variableName it's about) and, for a row inside a picker grid, an `index` too (multiple rows share the same `<picker>.select` variable name, so match on both). `FieldControl` accepts a `messages` prop for exactly this — filter the session's `messages` by `field` (and `index` where relevant) and pass the matching subset to each field/row as you render it. Compute this as a plain pass over the fields/rows you're about to render, not as a side effect during JSX construction. Don't show a generic top-of-page list for messages that don't match a rendered field — unmatched messages are simply not shown.
