...
- **Mapping the BOM into ServiceNow quote line items needs to exclude container/category nodes, or costs double-count** — `products` includes structural grouping nodes (e.g. "Cash Management," "Add-Ons") alongside real sellable line items; a container's `rollUpPrice` is just the sum of its children's prices, so submitting both the container and its children as separate quote lines double- (or triple-) counts the cost. Detect a container by checking whether any *other* product in the BOM lists it as `parentProduct` — the root product (matched by `partnerId === ` the configured product ID) is always a real line item regardless. See the comment in the template's `ConfiguratorDemo.jsx` `handleSubmit` TODO for the exact filter. Also expect many BOM item names (bundle components, add-ons) to have no matching `sn_prd_pm_product_offering` record by that name — that's a catalog-modeling question specific to each demo's SN instance, not a bug in this code; the ServiceNow route leaves `product_offering` blank rather than failing the whole quote when no match is found.
